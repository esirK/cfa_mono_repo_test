...
gunicorn=$(rlocation "cfalib/projects/twoopstracker/manage_gunicorn")
if [[ ! -f "${gunicorn:-}" ]]; then
  echo >&2 "ERROR: could not look up the gunicorn tool path"
  exit 1
fi

celery=$(rlocation "cfalib/projects/twoopstracker/manage_celery")
if [[ ! -f "${celery:-}" ]]; then
  echo >&2 "ERROR: could not look up the gunicorn tool path"
  exit 1
fi
